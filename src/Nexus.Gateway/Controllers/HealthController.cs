...
public class HealthController : ControllerBase
{
    ...
    public async Task<IActionResult> GetHealth([FromQuery] bool detailed = true)
    {
        ...
        try
        {
            ...
            {
                dbStatus = "Degraded";
                dbDetails = "Cannot connect to configured database provider";
            }
            else
            {
        ...
        try
        {
            var res = await _ps.ExecuteAsync("-NoProfile -Command \"$PSVersionTable.PSVersion.ToString()\"", HttpContext.RequestAborted);
            psSw.Stop();
            if (res.ExitCode == 0)
            {
            ...
            {
                psStatus = "Degraded";
                psDetails = $"PowerShell exited with code {res.ExitCode}: {res.StandardError}";
            }
        }
        catch (Exception ex)
        {
            psSw.Stop();
            psStatus = "Unhealthy";
            psDetails = $"PowerShell engine error: {ex.Message}";
        }
        ...
        catch (Exception ex)
        {
            cimSw.Stop();
            cimStatus = "Degraded";
            cimDetails = $"CIM WMI error: {ex.Message}";
        }
        ...
        var activeJobs = _jobManager.GetAllJobs().Count();
        jobSw.Stop();
