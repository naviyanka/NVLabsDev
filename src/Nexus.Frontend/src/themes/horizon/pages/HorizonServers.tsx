...
import { getServersClient, addServerClient, deleteServerClient, editServerClient, type Server } from "@/api/client";
import { Server as ServerIcon, Plus, Edit, Trash2, Terminal, Monitor, X } from "lucide-react";
...
export function HorizonServers() {
  ...
  if (loading) return <div className="text-xs text-[var(--text-sub)]">Loading Horizon Fleet…</div>;
    ...
    <div className="max-w-[1600px] mx-auto space-y-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-sub)] mt-1">Manage and monitor compute resources across all availability zones.</p>
        </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 w-28">
                        ...
                        <div className="h-1.5 flex-1 bg-[var(--border-dim)] rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${srv.cpu}%`, backgroundColor: srv.cpu > 80 ? "var(--crit)" : srv.cpu > 50 ? "var(--warn)" : "var(--amber)" }}></div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 w-28">
                        ...
                        <div className="h-1.5 flex-1 bg-[var(--border-dim)] rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${srv.mem}%`, backgroundColor: srv.mem > 80 ? "var(--crit)" : srv.mem > 50 ? "var(--warn)" : "var(--teal)" }}></div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 w-28">
                        ...
                        <div className="h-1.5 flex-1 bg-[var(--border-dim)] rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${srv.disk}%`, backgroundColor: srv.disk > 80 ? "var(--crit)" : srv.disk > 50 ? "var(--warn)" : "var(--amber)" }}></div>
                        </div>
                      </div>
                    </td>
